...
public class UP_F09_Pesquisar_Cliente extends javax.swing.JDialog {
    ...
    private void preencherTabela() {
        ...
        if (txtPesquisaCliente.getText().equals("")) {
            sql = "select idCliente, nome, snome from clientes";
        } else {
            if (RadioNomeCliente.isSelected()) {
                sql = "select idCliente, nome, snome from clientes " + " where nome like '" + txtPesquisaCliente.getText() + "%'";
            }
            if (RadioNomeCliente.isSelected()) {
                sql = "select idCliente, nome, snome from clientes " + " where snome like '" + txtPesquisaCliente.getText() + "%'";
            }
            if (RadioNomeCliente.isSelected()) {
                sql = "select idCliente, nome, snome from clientes " + " where idCliente like '" + txtPesquisaCliente.getText() + "%'";
            }

            ResultSet rs = dadosDB.getConsulta(sql);
            try {
                while (rs.next()) {
                ...
                Logger.getLogger(UP_F09_Pesquisar_Cliente.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
